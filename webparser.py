...
def get_schedule() -> list:
    data = []
    url = (
        "https://mai.ru/education/schedule/detail.php?group=%D0%9C4%D0%9E-112%D0%91-21"
    )
    ...
    for day in schedule_content.find_all(class_="sc-container"):
        ...
        for item in day.find("div", class_="sc-table sc-table-detail").find_all(
            "div", class_="sc-table-row"
        ):
            ...
            tutor = " | ".join(
                [
                    name.text.strip()
                    for name in item.find("div", class_="sc-item-title-body").find_all(
                        "span", class_="sc-lecturer"
                    )
                ]
            )
            location = item.find_all(class_="sc-table-col sc-item-location")[
                1
            ].text.strip()
            day_data += [
                {
